...
public class PersonFetcher {
	...
	private PersonFetcher(final Builder builder) {
	 ...
	 *
	 */
	public static class Builder extends AbstractFetcher<Builder, PersonFetcher> {
		...
		public Builder addFacultyFilter(final Faculty faculty) {
			addFilter(new IFilter<PersonFetcher>() {
				@Override
				public boolean apply(final PersonFetcher data) {
		...
		@Override
		protected PersonFetcher onCreateItem(final String rootPath) throws Exception {
			// reset Variables...
			mSex = null;
			mFaculty = null;
			mStatus = null;
