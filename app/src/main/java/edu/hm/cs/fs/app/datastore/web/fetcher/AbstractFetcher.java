 ...
 *            The Content type
 */
public abstract class AbstractFetcher<B extends AbstractFetcher<B, T>, T> {
	private static final String TAG = AbstractFetcher.class.getSimpleName();
	...
	protected AbstractFetcher(final Context context, final String url,
	...
	public final List<T> build() throws Exception {
		...
		if (!isOnline() && mOfflineFile == null) {
			// No internet connection and no offline file --> return empty list
			return content;
		}
